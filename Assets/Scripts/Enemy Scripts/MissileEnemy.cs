...
{
    public EnemyMissile missilePrefab; //The missile prefab the enemy shoots
    ...
    float delayBetweenMissiles = 3.5f;
    ...
    void Start()
    {
        ...
        speed = 1.5f;
        Invoke(nameof(Shoot), delayBetweenMissiles); //Make the enemy shoot once every few seconds
    }
    ...
    void Shoot() //Spawning a missile
    {
        ...
        if(Random.Range(0f, 1f) > 0.4f)
            Instantiate(missilePrefab, new Vector3(transform.position.x, transform.position.y, transform.position.z + 0.5f), Quaternion.identity);
